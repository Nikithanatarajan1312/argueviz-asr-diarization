...
def main():
    ...
    _vad_in_speech    = [False] # VAD state for status (set in ingest_loop)
    _asr_processing   = [False] # True while Whisper is running
    _low_energy_skips = [0]
    ...
    def status_broadcast_loop():
        while not _stop.is_set():
            ...
            if _asr_processing[0]:
                mode = "processing"
            elif _vad_in_speech[0]:
                mode = "recording"
            else:
                mode = "listening"
            _broadcast_status({
